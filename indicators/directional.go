...
func MinusDI(inHigh, inLow, inClose []float64, inTimePeriod int) []float64 {
	return plusminusDI(inHigh, inLow, inClose, inTimePeriod, true)
}

// PlusDI - Plus Directional Indicator
func PlusDI(inHigh, inLow, inClose []float64, inTimePeriod int) []float64 {
	return plusminusDI(inHigh, inLow, inClose, inTimePeriod, false)
}

func plusminusDI(inHigh, inLow, inClose []float64, inTimePeriod int, minusDI bool) []float64 {
	...
	for i != 0 {
		...
		if minusDI {
			if (diffM > 0) && (diffP < diffM) {
				prevPlusDM = prevPlusDM - (prevPlusDM / float64(inTimePeriod)) + diffM
			} else {
